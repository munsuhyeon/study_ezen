...
const txtArr = ['web publisher', 'front-end devloper', 
...
const headerEl = document.querySelector("header");
 // y좌표기준 즉, 세로로 1px이라도 스크롤이 화면에서 움직였는지 확인해서 움직인 값을 browserScrollY에 저장

window.addEventListener('scroll',function(){ // 화면에서 스크롤이 발행하면 함수 실행
    this.requestAnimationFrame(scrollCheck); // 화면 스크롤이 얼마나 움직였는지 확인 (scrollCheck 함수 정의해서 사용)
    //requestAnimationFrame 함수는 시스템이 프레임을 그릴 준비가 되면 애니메이션 프레임을 호출하여 애니메이션 웹페이지를 보다 원활하고 효율적으로 생성할 수 있도록 해준다. 
})
let scrollCheck = () => {
    let browserScrollY = window.pageYOffset;
...
// 부드러운 세로 이동 애니메이션 효과 구현
const animationMove = function(selector){
    const targetEl = document.querySelector(selector); // selector변수에 속하는 태그 선택해서 targetEl에 저장
    const browserScrollY = window.pageYOffset; // 웹사이트 스크롤값 정보
    const targetScrollY = targetEl.getBoundingClientRect().top + browserScrollY; // 헤더메뉴 선택하면 이동할 세로위치 값 저장  elem.getBoundingClientRect().top : 현재 창기준 세로 시작점 부터 엘리먼트 윗변 까지의 거리
    window.scrollTo({top:targetScrollY,behavior:'smooth'}); // targetScrollY에 저장된 값을 객체데이터인 {}에서 불러내서 window.scrollTo 명령어를 통해 원하는 위치로 이동
    // window.scrollTo(x좌표, y좌표)
    // window.scrollTo({top:0, left:0, behavior:'auto'}); behavior: auto, instant, smooth
}

// 이동 시 애니메이션 효과 data-animation-scroll="true" 구문이 들은 모든 태그선택
const scrollMoveEl = document.querySelectorAll("[data-animation-scroll='true']");
for(let i of scrollMoveEl){
    // 애니메이션 효과를 줄 모든 요소를 확인해서
    // 마우스 클릭 효과가 발생하면 해당 태그에 해당하는 section으로 좌표이동하는 함수 실행
